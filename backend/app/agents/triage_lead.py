...
class TriageLead:
    ...
    def orchestrate(self, bug: BugReport) -> TriageResult:
        ...
        # Step 3: Synthesize final severity
        severity_order = {"critical": 4, "high": 3, "medium": 2, "low": 1}
        tech_severity = severity_order.get(tech_output.decision, 2)
        business_severity = severity_order.get(business_output.decision, 2)
        weighted = (tech_severity * 0.6 + business_severity * 0.4)
        final_severity = {4: "critical", 3: "high", 2: "medium", 1: "low"}.get(round(weighted), "medium")
